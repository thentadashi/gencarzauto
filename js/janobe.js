...
function totalprice() {
  ...
  var sum = 0;
  for (var i = 0; i < items.length; i++) {
    var price = parseFloat(items[i].innerHTML.replace(/[^\d.]/g, ''));
    sum += price;
    items[i].textContent = formatNumber(price); // Format individual item totals with commas and 2 decimal places
  }

  var output = document.getElementById('sum');
  output.textContent = formatNumber(sum); // Format sum with commas and 2 decimal places
  ...
  if (deliveryfee.checked == true) {
    document.getElementById('fee').textContent = document.getElementById('PLACE').value;
  } else if (pickupfee.checked == true) {
    document.getElementById('fee').textContent = 0.0;
  }

  var overall = sum + parseFloat(fee);

  var overallprice = document.getElementById('overall');
  overallprice.textContent = formatNumber(overall); // Format overall with commas and 2 decimal places

  document.getElementById("alltot").value = overallprice.textContent;
}
...
$(document).on("click", ".paymethod", function() {
  ...
  var sum = 0;
  for (var i = 0; i < items.length; i++) {
    sum += parseFloat(items[i].value.replace(/,/g, ''));
  }
  ...
  var fee = parseFloat(delfee.value.replace(/,/g, ''));

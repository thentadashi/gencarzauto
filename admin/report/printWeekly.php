    ...
    <table>
        ...
        <tbody>
            <?php
						...
						WHERE S.ORDEREDSTATS = 'Delivered' or S.ORDEREDSTATS = 'PAID' AND S.ORDEREDDATE >= DATE(NOW()) - INTERVAL 7 DAY";
                        $totalQty = 0;
                        $totalAmount = 0;
                        ...
                        foreach ($curs as $result) {
                            ...
                            $query = "SELECT SUM(DELFEE) + SUM(PAYMENT) as Total FROM `tblsummary` where ORDEREDSTATS = 'Delivered' or ORDEREDSTATS = 'PAID' AND ORDEREDDATE >= DATE(NOW()) - INTERVAL 7 DAY";
                            $mydb->setQuery($query);
